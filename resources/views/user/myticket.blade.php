...
@endphp
    ...
    <div class="tab scrollmenu" style="border: none; border-bottom: 1px solid #F0F1F2;">
        @if(count($carts) > 0)
        <button class="tab-btn tablinks-myTickets active" onclick="openTabs(event, 'myTickets', 'cart')">Ticket Cart</button>
        @endif

        @if(count($payments) > 0 && count($carts) == 0)
        <button class="tab-btn tablinks-myTickets active" onclick="openTabs(event, 'myTickets', 'notPay')">Unpaid</button>
        ...
        <button class="tab-btn tablinks-myTickets"  onclick="openTabs(event, 'myTickets', 'For-Friends')">For Friends</button>
    </div>
                ...
                @forelse ($purchases as $purchase)
                    ...
                    <div class="bagi bagi-3 row-ticket mb-4">
                        ...
                        <div class="ticket" id="pilih-ticket" ticket-id="">
                            <div style="display: flex;">
                                @if ($purchase->events->start_date > $tanggalsekarang)
                                    ...
                                    <div class="teks-upcoming">
                                        <p class="teks-tebal" style="font-size: 75%!important; margin-top: 3px;">Upcoming</p>
                                    </div>
                                ...
                                @php
                                    $totalInviteUser = Invitation::where('ticket_id', $purchase->tickets->id)->where('sender', $purchase->send_from)->get();
                                            ...
                                            if($purchase->send_from == $myData->id){
                                                echo('
                                                    <div class="teks-send-to ml-1" >
                                                        <p class="teks-tebal" style="font-size: 75%!important; margin-top: 3px;">'.$purchase->users->name.'</p>
                                                    </div>
                                                ');
                                            }
                                        }
                            ...
                            <p class="teks-transparan teks-kecil desc-ticket" style="position: absolute">{{$purchase->tickets->description}}</p>
                            <div class="mt-5">
                                ...
                                <a class="bawah-garis" href="">
                                    <p class="teks-gelap mb-0 teks-bawah-garis font-weight-bold" >{{$purchase->events->name}}</p>
                                </a>
                                <p class="teks-transparan teks-kecil">Diadakan Oleh <b>{{$purchase->events->organizer->name}}</b></p>
                            </div>                    
                        </div>
                        </a>
