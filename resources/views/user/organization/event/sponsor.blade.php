...
<div class="row">
    <div class="col-md-7">
        <h2>Sponsor</h2>
        <h4 class="mt-3" style="color: #979797; font-size:14">Create and manage sponsors for your event</h4>
    </div>
    <div class="col-md-5">
        <button class="btn ke-kanan primer font-inter" onclick="confirmMyPkg()">
            <i class="fas fa-plus"></i> Sponsor Baru
        </button>
    </div>
</div>
...
@if(count($sponsors) == 0)
    <div class="mt-5 rata-tengah">
        <i class="fa fa-tv font-img teks-primer mb-3"></i>
        <h3>Tambahkan sponsor untuk Eventmu</h3>
...
@else
    @foreach ($sponsors as $sponsor)
        <div class="bagi bagi-3 mt-4">
            <div class="wrap">
                <div class="bg-putih bayangan-5" style="border-top-left-radius: 15px; border-top-right-radius: 15px;">
    
                    <div class="label bg-primer lable-title">{{ $sponsor->type }}</div>
                    
                    <i class="fa fa-ellipsis-h pointer dropdownToggle bagi-bagi-2 pt-2 pr-3 pl-1" data-id="{{ $sponsor->id }}" style=" color: #C4C4C4; font-size:20pt; float: right;" aria-hidden="true"></i>
                    <div id="Dropdown{{$sponsor->id}}" class="dropdown-content rounded-5">
                       
                        <a class="lebar-100 pointer" onclick='edit(<?= json_encode($sponsor) ?>, <?= json_encode($event) ?>)'>
                            <span class="teks-hijau"><i class="fas fa-edit"></i> Edit</span>
                        </a>
                        
                        <form action="{{ route('organization.event.sponsor.delete', [$organizationID, $event->id, $sponsor->id]) }}" method="POST">
                            @csrf
                            @method('delete')
                    ...
                    <div class="smallPadding">
                        <div class="wrap mb-0">
                            <div class="row">
                                <div class="col-12">
                                    <div class="w-100 asp-rt-5-2" bg-image="{{ asset('storage/event_assets/'.$event->slug.'/sponsor_logo/'.$sponsor->logo) }}"></div>
                                </div>
                                <div class="col-12 bottom-nav" style="background-color: #000000a6; color: white; display: flex; padding: 10px; border-bottom-left-radius: 15px; border-bottom-right-radius: 15px;">
                                    <div class="row w-100" style="margin: unset;">
                                        <div class="col-4 text-center my-auto">
                                            <a href="{{route('organization.event.sponsor.decrease', [$organizationID, $event->id, $sponsor->id])}}">
                                                <i class="bi bi-arrow-left-square text-light"></i>
                                            </a>
                                        </div>
                                        <div class="col-4 text-center my-auto">
                                            {{$sponsor->priority}}
                                        ...
                                        <div class="col-4 text-center my-auto">
                                            <a href="{{route('organization.event.sponsor.increase', [$organizationID, $event->id, $sponsor->id])}}">
                                                <i class="bi bi-arrow-right-square text-light"></i>
                                            </a>
                                        </div>
                                        <div class="col-12">
                                            <div class="text-center mt-3">
                                                <a href="{{ $sponsor->website }}" target="_blank" class="text-light">
                                                    See About Sponsor
                                                </a>
                                            </div>
                                        </div>
...
<div class="popupWrapper" id="addSponsor">
    <div class="popup rounded-5 lebar-70">
        <div class="wrap">
            <h3 class="rata-tengah mb-4">Sponsor Baru
                ...
                <div class="row">
                    ...
                    <div class="col-xl-6">
                        <div class="wrap">
                            ...
                            <select name="type" class="box no-bg" required oninvalid="this.setCustomValidity('Harap Pilih Tipe Sponsor Yang Ada Di List')" oninput="setCustomValidity('')">
...
<div class="popupWrapper" id="editSponsor">
    <div class="popup rounded-5 lebar-70">
        <div class="wrap">
            <h3 class="rata-tengah mb-4">Edit Sponsor
...
<script>
    ...
    const edit = (data, event = null) => {
        ...
        select("#editSponsor #logoPreview").setAttribute('src', `{{ asset('storage/event_assets/${event.slug}/sponsor_logo/${data.logo}') }}`);
    }
    function confirmMyPkg() {
        ...
        if(myPkg.sponsor_count <= -1){
            ...
            if(thisEvent.sponsors.length >= myPkg.sponsor_count){
                paramCombine = true;
            }
        }
        if(pkgActive == 0 || paramCombine == true){
            ...
            if(paramCombine == true){
                msg = 'Kamu sudah melewati batas paket untuk membuat sponsor baru';
            }
            else{
